...
if [ ! -f genbindings.sh ]; then
	echo 'genbindings.sh must be run from its root path'
	exit 1
fi
...
GOOS=android GOARCH=arm gobind -lang=go github.com/ChrisSmith/libcats > $GO_CATS/go_libcats.go
echo 'generating java binding'
GOOS=android GOARCH=arm gobind -lang=java github.com/ChrisSmith/libcats > $JAVA_DIR/libcats/Libcats.java
